...
const FeatureIcons = [
  <LuLayoutTemplate key={1} size={32} />,
  <LuZap key={2} size={32} />,
  <LuShieldCheck key={3} size={32} />,
  <LuUsers key={4} size={32} />,
];
    ...
    <div className="flex flex-col w-full bg-background min-h-screen">
      ...
      <div className="relative w-full h-[40vh] max-md:h-[35vh]">
        ...
        <div className="absolute items-center inset-0 flex flex-col px-4 max-w-5xl mx-auto mt-auto mb-16 h-max">
          <SpanL
            upperCase
            className="text-secondary text-sm bg-surface px-3 py-1 font-bold tracking-widest mb-3 w-max"
          >
            Services.label
          </SpanL>
          <SpanL className="text-6xl font-bold text-on-primary max-md:text-4xl drop-shadow-xl">
      ...
      <div className="flex flex-col self-center w-full max-w-5xl px-8 pt-16 pb-20 gap-20 max-md:px-5 max-md:py-12 max-md:gap-12">
        ...
        <div className="flex flex-col gap-6">
          <SpanL className="font-light text-lg leading-relaxed text-on-background max-md:text-2xl">
            {`Services.${service.id}.introduction`}
          </SpanL>
          <div className="w-24 h-0.5 bg-secondary mx-auto mt-4" />
        </div>
        {/* 3. VISUAL HELPER: ICON GRID (Key Features) */}
        <div className="flex flex-col gap-8">
          <SpanL className="text-2xl font-semibold text-primary">
          ...
          <div className="grid grid-cols-2 gap-8 max-md:grid-cols-1">
            {[0, 1, 2, 3].map((idx) => (
              ...
              >
                <div className="text-secondary bg-secondary/10 p-3 rounded-full shrink-0">
                  {FeatureIcons[idx]}
                </div>
                <div className="flex flex-col gap-2">
                  <SpanL className="text-xl font-medium text-primary">
                    {`Services.${service.id}.features.${idx}.title`}
                  </SpanL>
                  <SpanL className="text-on-background/70 leading-relaxed">
                    {`Services.${service.id}.features.${idx}.description`}
                  </SpanL>
                </div>
              </motion.div>
            ))}
        ...
        <div className="flex flex-col gap-6 text-center">
          <div className="w-24 h-0.5 bg-secondary mx-auto mt-4" />
          <SpanL className="text-3xl font-light leading-relaxed text-primary max-md:text-2xl">
            {`Services.${service.id}.longDescription`}
          </SpanL>
          <div className="w-24 h-0.5 bg-secondary mx-auto mt-4" />
        </div>

import { SpanL } from "@/components/ui";
import { FileText, ClipboardList, HardHat, CheckCircle } from "lucide-react";
import { StepItem } from "../Quality.types";
import { Flow } from "./Flow";

const QualitySteps: StepItem[] = [
  {
    id: "Preparation",
    icon: <FileText size={20} />,
    pointCount: 3,
  },
  {
    id: "Inspection",
    icon: <ClipboardList size={20} />,
    pointCount: 3,
  },
  {
    id: "Execution",
    icon: <HardHat size={20} />,
    pointCount: 3,
  },
  {
    id: "Reporting",
    icon: <CheckCircle size={20} />,
    pointCount: 3,
  },
];

export const Process = () => {
  return (
    <div className="my-32 mx-[6%] flex flex-col gap-2 max-2xl:mx-[2%]">
      <SpanL className="text-4xl self-center text-center text-primary">
        Quality.Process.title
      </SpanL>
      <SpanL className="self-center text-center tracking-wider font-light w-1/2 max-2xl:w-2/3">
        Quality.Process.text
      </SpanL>
      <div>
        <Flow steps={QualitySteps} />
      </div>
    </div>
  );
};
